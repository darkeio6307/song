...
window.onload = () => {
    ...
    if (savedUser) {
        bootSession(savedUser, true).then(() => clearTimeout(forceEnter)).catch(() => bypassBoot(savedUser));
    } else {
        setTimeout(() => { splash.classList.add('hidden'); login.classList.remove('hidden'); clearTimeout(forceEnter); }, 2500);
    }
};
...
function bypassBoot(u) {
    ...
    if(u) { currentUser = u; login.classList.add('hidden'); app.classList.remove('hidden'); fetchMusic("Top Hindi 50"); }
...
document.getElementById('loginBtn').onclick = async () => {
    ...
    if(!u || !p) return showToast("Details fill karo!");
...
function listenToGlobalFM() {
    onSnapshot(doc(db, "fm", "globalRadio"), (snap) => {
        ...
        if(d && d.isLive && d.host !== currentUser) {
            ...
            currentFMSongId = d.songId; // For chat listener badge
            ...
            // FM KILL SWITCH TRIGGERED FOR LISTENERS
...
// Ensure FM dies if Dark_eio closes app
...
document.getElementById('btnChatToggle').onclick = () => {
    ...
    // Load online users (Global + FM Listeners)
...
document.querySelectorAll('.mood-chip').forEach(btn => {
    btn.onclick = () => {
        ...
        fetchMusic(mood);
        showToast(`AI generating ${btn.innerText} vibes... 🎧`);
    };
});
